...
class Model(object):
    ...
    def _new_layer(self):
        return Model(self.lrn_rate, self.dsc_rate, self.min_likelihood, level=self.level+1)
...
class HeatMappedModel(Model):
    def __init__(self, lrn_rate, dsc_rate, min_likelihood, level=0):
        Model.__init__(self, lrn_rate, dsc_rate, min_likelihood, level=level)

    def _new_layer(self):
        #return HeatMappedModel(self.lrn_rate, self.dsc_rate, self.min_likelihood, level=self.level+1)
        return HeatMappedModel(self.lrn_rate, self.dsc_rate, .0 ** (self.level + 1), level=self.level+1)
